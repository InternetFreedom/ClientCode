...
def choose_proxy(client_info, user_filename, proxy_filename):
	...
	## Group2: '5', '2', '16', '1'
	## Group3: others
	if client_info[0] == '3' or client_info[0] == '4' or client_info[0] == '8' or client_info[0] == '18' or client_info[0] == '19' or client_info[1] == '3' or client_info[1] == '4' or client_info[1] == '8' or client_info[1] == '18' or client_info[1] == '19' :
	...
	while True:
		...
		while flag == 0:
			if os.path.exists(work_dir + '/user_data/client_info.txt'):
				# Delete old proxy file
				if os.path.exists(work_dir + '/proxy_data/proxy.txt'):
					os.system('sudo rm ' + work_dir + '/proxy_data/proxy.txt')
				flag = 1
				# if there is a successful connection, do 'stat'
				os.system("cd /root/Desktop/server_iodine/ && sudo python stat.py " + work_dir + "/.ssh/authorized_keys")
		...
		if flag == 1:
			...
			f1 = open(work_dir + '/user_data/client_info.txt','r')
			info = f1.readline()
			fw = open(work_dir + '/user_data/negative_survey.txt','a')
			fw.write(info+'\n')
			fw.close
			f1.close
